...
export const UserPosts = ({allUserPosts, setAllUserPost,loadAllPost, isLoaded, userProfilePicture}) => {
 ...
 // const [isLoaded, setIsLoaded] = useState(false);
  ...
  const [showAllComments,setShowAllComments]=useState(2)
  ...
  // userAllPosts=allUserPosts
  const showPostOption = (index) => {
    if (showOption == "false") {
      setShowOption(index + "true");
    } else {
      setShowOption("false");
    }
  };
  ...
  let showLimitComment=allComments
  const handleShowAllComment=(id,pc)=>{
    setShowAllComments(showAllComments+2)
  }
   const [reply,setReply]=useState("")
  
    const Reply =(postId,parentComment)=>{
   let data={
        postId,
        parentComment,
        comment
      }
      AddReply(data,linkedinId).then((res) =>{
        if(res !=undefined)
        {
          console.log(res)
          if(res.success)
    ...
    <>
        ...
        {(allUserPosts !==null && allUserPosts.length>0)&& 
            allUserPosts.map((val, ind) => {
          return (
              ...
              <div className="card-body">
                ...
                <ul className="nav nav-pills nav-pills-light nav-fill nav-stack small border-top border-bottom py-1 my-3">
                  <li className="nav-item">
                   ...
                   <a className={val.hasUserLiked?"nav-link mb-0 active":"nav-link mb-0"} onClick={(e)=>Like(val._id)} >
