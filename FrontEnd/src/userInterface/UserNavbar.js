...
export default function UserNavbar() {
  ...
  const Active = 'nav-link icon-md btn  p-0   d-block active'
  const normal = 'nav-link icon-md btn  p-0   d-block '
  ...
  const handleSerchButton = (e) => {
    ...
    try {
      if (searchValue != '') {
        search(searchValue.toLowerCase()).then((res) => {
          console.log(res.data.Users);
          if (res.success && res.data.Users.length !== 0) {
            localStorage.setItem("searchData", JSON.stringify(res.data.Users));
